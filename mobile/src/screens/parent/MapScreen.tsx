...
export default function MapScreen() {
  const { liveLocation, pairedChild, geofences, setGeofences, addGeofence, removeGeofence, setLiveLocation } = useStore();
  ...
  useEffect(() => {
    ...
    send({ t:'loc', lat:liveLocation.latitude, lng:liveLocation.longitude, name:(liveLocation as any).childName||'Child', ts:liveLocation.timestamp });
  }, [liveLocation, ready]);
  ...
  // When map becomes ready, fetch latest known location from REST if store is empty
  useEffect(() => {
    if (!ready) return;
    if (!liveLocation && pairedChild) {
      locationAPI.latest(pairedChild.id).then(({ data }) => {
        if (data?.latitude) {
          setLiveLocation({ ...data, childName: pairedChild.name, timestamp: data.recordedAt || new Date().toISOString() });
        }
      }).catch(() => {});
    }
  }, [ready]);
